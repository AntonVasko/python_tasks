...
class Cassa():
    ...
    def cassa_products(self):
        self.to_json = dict()
        for el in self.products:
            self.to_json[el] = dict()
            self.to_json[el]['Цена'] = self.products[el]
            self.to_json[el]['Скидка'] = str(self.discount[el]*100) + '%'
            self.to_json[el]['Цена со скидкой'] = round((1-self.discount[el])*self.products[el], 2)
